...
class Gesture:
    ...
    def calHandAngle(self, hand):
        ...
        for i in range(1, 5): 
            base = i * 4
            angle = self.calAngle(
                (int(hand[0][0]-hand[base+2][0]), int(hand[0][1]-hand[base+2][1])),
                (int(hand[base+3][0]-hand[base+4][0]), int(hand[base+3][1]-hand[base+4][1]))
            )
            handAngle.append(angle)
        # 食指與中指的夾角
        angle = self.calAngle(
    ...
    def detect(self):
        ...
        while(True):
            ...
            if result.multi_hand_landmarks:
                # 畫出來
                res = str(result.multi_handedness[0])# get the label to claim right or left
                label = res.split('"')
                for handLandmark in result.multi_hand_landmarks:
                    self.myDrawing.draw_landmarks(newFrame, handLandmark, self.myHand.HAND_CONNECTIONS)
                    ...
                    if handLocal:
                        ...
                        # 判斷手勢
                        gesture,act = self.judgeGesture(handAngle,angle,label[1])
                        # 在畫面中顯示文字(畫面, 文字, 位置, 字型, 大小(縮放比例), 顏色, 線條寬度, 線條種類))
                        cv2.putText(newFrame, gesture, (50, 50), 0, 1.3, (0,0,255), 3, cv2.LINE_AA)
                        cv2.putText(newFrame, act, (400, 50), 0, 1.3, (0,0,255), 3, cv2.LINE_AA)
